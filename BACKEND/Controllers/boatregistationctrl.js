...
const addboats = async (req, res, next) => {
    const { name, Numberof, phone, phone2 } = req.body;

    let newUser;

    try {
        newUser = new Boatdetails({ name, Numberof, phone, phone2});
        await newUser.save();
    } catch (err) {
        console.log(err);
...
const updateboats = async (req, res, next) => {
    ...
    try {
        updatedUser = await Boatdetails.findByIdAndUpdate(
            id,
            { name: name, Numberof: Numberof, phone: phone, phone2: phone2 }, // Ensure correct case
